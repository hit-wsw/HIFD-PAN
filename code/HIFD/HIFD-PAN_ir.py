...
def train(img2, pan, ms, ir, img1, max_iter):
    loss = 2*loss1(img2,pan,img1) + 3*loss2(img2, ms, img1, ir) + loss3(img2,pan, img1)
    ...
    while epoch <= max_iter:
        ...
        cv2.imwrite(f'results/pan_{epoch}_{loss.item()}.png', (result_img * 255).astype(np.uint8))'''
...
img1 = cv2.imread("./data/MS_resize.png", cv2.IMREAD_COLOR)
img2 = cv2.imread("./data/IR_resize.png", cv2.IMREAD_COLOR)
...
img2= train(img2, pan, ms, ir, img1, 750)
true_color_result = img2.detach().cpu().squeeze().permute(1, 2, 0).numpy()

cv2.imshow("false_color_result", true_color_result)
cv2.waitKey(0)
cv2.imwrite("false_color_result.png", true_color_result * 255)
#writer.close()
